...
reader's destinations are its unicast ports *and* the multicast groups it announces
([#196](https://github.com/atinfinity/fastdds_transport_viz/issues/196)), which is the rule
the measurement itself uses: a pair whose reader announces a group and no unicast locator is
measured through that group's entry and through no other, so counting unicast alone left it
unsettled to `--timeout` and warned although every packet had been attributed. Metatraffic
needs no exception there - `239.255.0.1:7400` is a *participant* locator, and no endpoint
announces it. The last two conditions are what a fixed window cut short
