...
| `--stats` coverage | ≥ 95 % | pairs with measured packets (`measured.packets > 0`, the same definition as the `--watch` coverage; a delivery proof alone does not count since [#153](https://github.com/atinfinity/fastdds_transport_viz/issues/153)) at `--timeout 5` among those at `--timeout 30`; for the synthetic loads only the `/scale` pairs count (`/parameter_events` often sends nothing within 5 s), for Nav2 every pair; 0 when the 30 s run measured none of them. The pairs with a delivery proof but no packet are recorded as `delivered_unmeasured_5s` / `_30s` and judged by nothing |
