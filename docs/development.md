...
| dropped statistics samples | 0 | growth of the `drain` phase's `sample_lost` - `sample_lost_latency` + `sample_rejected` after the first `--watch` frame, the higher of both runs. Since [#141](https://github.com/atinfinity/fastdds_transport_viz/issues/141) the `HISTORY_LATENCY` reader is best-effort by design and reports every sequence gap, so its losses are counted apart (`sample_lost_latency`, `stats.samples_lost_latency`) and not judged here: they coarsen a percentile, they cost no pair its measurement. A reader that matches a writer late counts the samples already gone from the writer's keep-last history as lost; since #134 the tool keeps those apart as `sample_lost_at_start` (`stats.samples_lost_at_start` in the document), so they no longer inflate this row. The one-shot `--stats` runs record them as `oneshot_lost_samples` ("at start" in the row) |
...
- **medium:**
  ...
  - the counter samples lost over a 60 s `--watch` run vary between runs of one build: 0 in the `--stats` run and 5823 in the `-v` run of the same row ([#147](https://github.com/atinfinity/fastdds_transport_viz/issues/147));
