...
| `!stats-samples-lost`, or `warning: ... statistics samples were lost` | the tool did not receive part of the statistics and at least one pair with proven deliveries shows no measured packet (`stats.pairs_delivered_unmeasured`), so a pair can read `(idle)` or `none(delivered)` although it carries traffic. A loss that cost no measurement shows as `N sample(s) lost` without this warning. Enable statistics on fewer nodes, or keep `FASTDDS_STATISTICS` to the aliases you need (`RTPS_SENT_TOPIC;RTPS_LOST_TOPIC` is enough for the transports). A longer `--timeout` does not help: it collects more of the loss, not less. `--json` counts them in `stats.samples_lost`, of which `stats.samples_lost_latency` is the best-effort `HISTORY_LATENCY` part that costs no pair its measurement and is left out of the warning; `stats.samples_lost_at_start` is the normal burst from before the readers matched and never warns. |
