...
latency of the pair (mean and max), `HZ` the samples per second that reached the reader
(`≥` when part of the statistics was lost and the value is a lower bound), `LOSS` lost and
resent packets, and hosts are shown by name with process ids. The profile file
...
| `!stats-samples-lost`, or `warning: ... statistics samples were lost` | the tool did not receive part of the statistics and at least one pair with proven deliveries shows no measured packet (`stats.pairs_delivered_unmeasured`), so a pair can read `(unmeasured, delivered)` or `none(delivered)` although it carries traffic. A loss that cost no measurement shows as `N sample(s) lost` without this warning. Enable statistics on fewer nodes, or keep `FASTDDS_STATISTICS` to the aliases you need (`RTPS_SENT_TOPIC;RTPS_LOST_TOPIC` is enough for the transports). A longer `--timeout` does not help: it collects more of the loss, not less. `--json` counts them in `stats.samples_lost`, of which `stats.samples_lost_latency` is the best-effort `HISTORY_LATENCY` part that costs no pair its measurement and is left out of the warning; `stats.samples_lost_at_start` is the normal burst from before the readers matched and never warns. |
...
| pairs the system does have are missing, or `warning: discovery was still in progress (...)` | the run stopped while the nodes were still announcing their endpoints: it ends after `--quiet` seconds without discovery events, and a large system falls silent between two batches. Re-run with the values the message names (`--quiet 3 --timeout 10` is a good start; with `--stats`, `--quiet 3 --timeout 60`). `--json` says the same in `discovery.complete` (`false`), and such a document is not worth diffing. |
