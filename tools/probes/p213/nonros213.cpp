// Issue #213 copy of tools/probes/p210/nonros210.cpp. Added env knobs:
//   P213_STRUCT_NAME  name of the DynamicType (TypeObject type_name); registered under P206_TYPE_NAME anyway
//   P213_MODE=header  std_msgs::msg::dds_::Header_ { <P213_NESTED_NAME> stamp; string frame_id; }
//   P213_NESTED_NAME  nested Time_ struct name (default builtin_interfaces::msg::dds_::Time_)
//   P213_TP           participant property fastdds.type_propagation (enabled|disabled|minimal_bandwidth|registration_only)
// Copy of tools/probes/p206/nonros_app.cpp extended for issue #210: per-sample source
// writer + decoded values, unread-vs-taken accounting (detects samples dropped by a failed
// deserialization inside take()), P210_NO_WRITER=1 / P210_NO_READER=1, end-of-run statuses.
...
static bool header_mode() { const char * m = std::getenv("P213_MODE"); return m && std::string(m) == "header"; }
static std::string str_member()
{
  if (header_mode()) { return "frame_id"; }
  return std::getenv("P206_MEMBER_NAME") ? std::getenv("P206_MEMBER_NAME") : "data";
}
static void apply_ext(TypeDescriptor::_ref_type & td)
{
...
static DynamicType::_ref_type build_time_type()
{
  ...
  td->name(std::getenv("P213_NESTED_NAME") ? std::getenv("P213_NESTED_NAME") : "builtin_interfaces::msg::dds_::Time_");
  apply_ext(td);
  DynamicTypeBuilder::_ref_type b{factory->create_type(td)};
  MemberDescriptor::_ref_type m0{traits<MemberDescriptor>::make_shared()};
  m0->name("sec"); m0->type(factory->get_primitive_type(TK_INT32)); b->add_member(m0);
  MemberDescriptor::_ref_type m1{traits<MemberDescriptor>::make_shared()};
  m1->name("nanosec"); m1->type(factory->get_primitive_type(TK_UINT32)); b->add_member(m1);
  return b->build();
}
...
static DynamicType::_ref_type build_string_type(bool with_extra)
{
  ...
  TypeDescriptor::_ref_type td{traits<TypeDescriptor>::make_shared()};
  td->kind(TK_STRUCTURE);
  td->name(std::getenv("P213_STRUCT_NAME") ? std::getenv("P213_STRUCT_NAME") : kTypeName.c_str());
  apply_ext(td);
  DynamicTypeBuilder::_ref_type b{factory->create_type(td)};
  if (header_mode()) {
    MemberDescriptor::_ref_type ms{traits<MemberDescriptor>::make_shared()};
    ms->name("stamp"); ms->type(build_time_type()); b->add_member(ms);
  }
...
public:
  ...
  {
    ++callbacks_;
    const uint64_t unread = r->get_unread_count(false);
    unread_seen_ += unread;
    int taken_now = 0;
    DynamicData::_ref_type d{DynamicDataFactory::get_instance()->create_data(type_)};
    SampleInfo info;
    ReturnCode_t rc;
    while (RETCODE_OK == (rc = r->take_next_sample(&d, &info))) {
      ++taken_now;
      if (info.valid_data) {
        ++received_;
        const auto & wg = info.sample_identity.writer_guid();
        char pre[64];
        std::snprintf(pre, sizeof(pre), "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
          wg.guidPrefix.value[0], wg.guidPrefix.value[1], wg.guidPrefix.value[2], wg.guidPrefix.value[3],
          wg.guidPrefix.value[4], wg.guidPrefix.value[5], wg.guidPrefix.value[6], wg.guidPrefix.value[7],
          wg.guidPrefix.value[8], wg.guidPrefix.value[9], wg.guidPrefix.value[10], wg.guidPrefix.value[11]);
        std::string key(pre);
        if (per_writer_[key]++ < 2) {
          std::string str;
          d->get_string_value(str, d->get_member_id_by_name(str_member()));
          int32_t extra = -12345;
          std::string ex = "n/a";
          if (type_->get_member_count() > (header_mode() ? 2u : 1u)) {
            d->get_int32_value(extra, d->get_member_id_by_name("extra"));
            ex = std::to_string(extra);
          }
          std::cout << "# DATA reader " << tag_ << " from=" << key << " data=\"" << str
                    << "\" (len " << str.size() << ") extra=" << ex << std::endl;
        }
      }
    }
    if (static_cast<uint64_t>(taken_now) < unread) {
      dropped_ += unread - taken_now;
      if (drop_logs_++ < 3) {
        std::cout << "# DROP reader " << tag_ << ": unread=" << unread << " taken=" << taken_now
                  << " (take rc=" << static_cast<int>(rc) << ")" << std::endl;
      }
    }
  }
  std::map<std::string, int> per_writer_;
  int callbacks_{0};
  uint64_t unread_seen_{0};
  uint64_t dropped_{0};
  int drop_logs_{0};
  std::string tag_;
...
static bool make_side(Side & s, int domain, const char * pname, bool with_extra, bool want_reader)
{
  ...
  pqos.name(pname);
  if (const char * tp = std::getenv("P213_TP")) {
    pqos.properties().properties().emplace_back("fastdds.type_propagation", tp);
    std::cout << "# fastdds.type_propagation=" << tp << std::endl;
  }
  ...
  if (RETCODE_OK != ts.register_type(s.p, kTypeName)) {
...
int main(int argc, char ** argv)
{
  ...
  auto write_one = [](Side & s, int i) {
      ...
      d->set_string_value(d->get_member_id_by_name(str_member()), "hello from " + s.wl->tag_ + " " +
      std::to_string(i));
      if (s.type->get_member_count() > (header_mode() ? 2u : 1u)) {
  ...
  for (Side * s : {&a, &b}) {
    if (s->r) {
      SampleLostStatus sl; s->r->get_sample_lost_status(sl);
      SampleRejectedStatus sr; s->r->get_sample_rejected_status(sr);
      SubscriptionMatchedStatus sm; s->r->get_subscription_matched_status(sm);
      std::cout << "# END reader " << s->rl->tag_ << ": callbacks=" << s->rl->callbacks_
                << " unread_seen=" << s->rl->unread_seen_ << " taken_valid=" << s->rl->received_
                << " dropped_in_take=" << s->rl->dropped_ << " sample_lost=" << sl.total_count
                << " sample_rejected=" << sr.total_count << " matched=" << sm.current_count << std::endl;
      for (auto & kv : s->rl->per_writer_) {
        std::cout << "#   from " << kv.first << ": " << kv.second << std::endl;
      }
    }
  }
