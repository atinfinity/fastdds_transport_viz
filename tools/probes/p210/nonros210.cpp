    ...
    while (RETCODE_OK == (rc = r->take_next_sample(&d, &info))) {
      ...
      if (info.valid_data) {
        ...
        if (per_writer_[key]++ < 2) {
          std::string str;
          d->get_string_value(str, d->get_member_id_by_name(
            std::getenv("P206_MEMBER_NAME") ? std::getenv("P206_MEMBER_NAME") : "data"));
          int32_t extra = -12345;
          std::string ex = "n/a";
          if (type_->get_member_count() > 1) {
...
int main(int argc, char ** argv)
{
  ...
  auto write_one = [](Side & s, int i) {
      ...
      d->set_string_value(d->get_member_id_by_name(
        std::getenv("P206_MEMBER_NAME") ? std::getenv("P206_MEMBER_NAME") : "data"), "hello from " + s.wl->tag_ + " " +
      std::to_string(i));
