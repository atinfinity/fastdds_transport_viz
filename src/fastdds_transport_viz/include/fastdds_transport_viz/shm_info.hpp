...
/// number is one of the tool's own), Visible when every port is held here and one of them
/// is in `proof_ports` (announced by an endpoint other than its `ros_discovery_info`
/// reader) with no other participant announcing it in `participants_per_port`, otherwise
/// Unprobed (no scan, a lock that could not be read, no such port). A number shared by
/// participants in several IPC namespaces does not say whose lock is held, and neither does
/// the reader's 7000+ number, which any Fast DDS participant of the namespace may take,
/// whatever its domain: such ports leave the decision to the others.
