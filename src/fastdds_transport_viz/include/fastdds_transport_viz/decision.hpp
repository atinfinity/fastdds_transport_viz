...
/// matched by its GUID. The keys of `added` and `changed` are the after snapshot's, those
/// of `removed` the before snapshot's; PairChange::before_key carries the before identity.
/// Pure function; `Changes::before` is left for the caller.
