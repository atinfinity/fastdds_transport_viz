...
ENV = {
    'FASTRTPS_DEFAULT_PROFILES_FILE': os.path.join(
        os.path.dirname(__file__), 'multicast_user.xml')}
...
class TestMulticastLocators(Base):
    ...
    def test_multicast_locator_reported(self):
        ...
        for ep in chatter['writers'] + chatter['readers']:
            kinds = [
                (loc['kind'], loc['address'], loc['port']) for loc in ep['multicast_locators']]
        ...
        unicast = {
            loc['kind'] for ep in chatter['writers'] + chatter['readers']
