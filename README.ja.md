...
同じ実行結果を [web viewer](docs/web-viewer.ja.md) で開いたところ (テーブルビュー):

![table view](docs/images/web-viewer-table.jpg)
...
できます (`:humble`、`:jazzy`、`:lyrical`。[はじめに](docs/getting-started.ja.md#docker-prebuilt-image-ghcr) を参照):
...
結果を強調表示します。詳細は
[docs/how-it-works.ja.md](docs/how-it-works.ja.md#2-つのスナップショットの比較)。
...
- [開発・検証・テスト](docs/development.md) (英語) — Docker 環境、パッケージ構成、検証ノード、マルチコンテナのシナリオ、テスト、規模の検証の手順、リリース手順、ロードマップ
- [検証ログ](docs/verification-log.md) (英語) — 日付つきの規模の検証結果と検証の記録
...
- **type hash には ROS 2 Jazzy 以降が必要です。** 型 *名* が違う writer と reader は `NONE` と
  ...
  (つまり Jazzy) も同様に広告しません (`docs/verification-log.md` 参照)。Humble では、すべてのノードを
